      ...
      <div className="border-b px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-gray-900 border border-gray-300 px-4 py-2 rounded"
          >
          ...
          >
            {sorts.map(function(s) { return <option key={s}>{s}</option>; })}
          </select>
        </div>
      </div>
      ...
      <div className="flex">
        {showFilters && (
          <div className="w-64 flex-shrink-0 border-r px-6 py-6">
                ...
                {prices.map(function(price) {
                  return (
                    <label key={price} className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="price" checked={selectedPrice === price} onChange={function() { setSelectedPrice(price); }} className="accent-gray-900" />
                ...
                {colors.map(function(color) {
                  return (
                    <label key={color} className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="color" checked={selectedColor === color} onChange={function() { setSelectedColor(color); }} className="accent-gray-900" />
            ...
            <button
              onClick={function() { setSelectedCategory('全部'); setSelectedPrice('全部'); setSelectedColor('全部'); }}
        ...
        <div className="flex-1 px-8 py-6">
          {loading ? (
            ...
            <div className={'grid gap-6 ' + (showFilters ? 'grid-cols-3' : 'grid-cols-4')}>
              {filteredProducts.map(function(product) {
                return <ProductCard key={product.id} product={product} onClick={function() { navigate('/products/' + product.id); }} />;
              })}
            </div>
          )}
          {!loading && filteredProducts.length === 0 && (
            <div className="text-center py-24">
              ...
              <button onClick={function() { setSelectedCategory('全部'); }} className="mt-4 text-gray-900 underline">查看全部产品</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="cursor-pointer group" onClick={onClick}>
      <div className="relative bg-gray-100 rounded-lg overflow-hidden mb-3">
        ...
        />
        <button
          onClick={function(e) { e.stopPropagation(); setFavorite(!favorite); }}
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill={favorite ? '#b91c1c' : 'none'} viewBox="0 0 24 24" stroke={favorite ? '#b91c1c' : 'currentColor'}>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
          </svg>
        </button>
        <div className="absolute top-3 left-3">
          <span className="bg-white text-gray-900 text-xs font-bold px-2 py-1 rounded">新品</span>
        </div>
      </div>
      <div>
