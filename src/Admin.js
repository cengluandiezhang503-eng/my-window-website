    ...
    <div style={{ display: 'flex', height: '100vh', background: '#0a0a0f', color: '#e0e0e8', fontFamily: 'system-ui, sans-serif', fontSize: '13px' }}>
      ...
      <div style={{ width: '220px', flexShrink: 0, background: '#0d0d14', borderRight: '1px solid #1e1e2e', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 16px', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '32px', height: '32px', background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🏠</div>
          <div>
        ...
        <div style={{ flex: 1, padding: '12px 0' }}>
          <div style={{ padding: '8px 16px 4px', fontSize: '10px', color: '#3a3a5a', letterSpacing: '2px' }}>主菜单</div>
          {[
            ...
            return (
              <div key={item.id} onClick={function() { setActiveNav(item.id); }}
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 16px', cursor: 'pointer', color: activeNav === item.id ? '#818cf8' : '#6060a0', background: activeNav === item.id ? '#13132a' : 'transparent', borderLeft: activeNav === item.id ? '2px solid #818cf8' : '2px solid transparent' }}>
                <span>{item.icon}</span>
          ...
          <div style={{ padding: '8px 16px 4px', marginTop: '8px', fontSize: '10px', color: '#3a3a5a', letterSpacing: '2px' }}>系统</div>
          {[
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ height: '56px', background: '#0d0d14', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '8px', padding: '6px 12px', width: '200px' }}>
            ...
            <input value={searchQuery} onChange={function(e) { setSearchQuery(e.target.value); }} placeholder="搜索..."
            ...
            <div>
              ...
              <div style={{ background: '#0d0d14', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['客户姓名', '邮箱', '产品类型', '状态', '时间'].map(function(h) {
                    ...
                    {quotes.slice(0, 5).map(function(quote) {
                      return (
                        <tr key={quote.id}>
                          ...
                          <td style={{ padding: '12px 16px', color: '#6060a0', borderBottom: '1px solid #13131e', fontSize: '12px' }}>{quote.email}</td>
          ...
          {activeNav === 'products' && (
            <div>
                    ...
                    {[
                      ...
                      return (
                        <div key={field.key}>
                          ...
                          <input value={newProduct[field.key]} onChange={function(e) { setNewProduct(Object.assign({}, newProduct, { [field.key]: e.target.value })); }}
              ...
              <div style={{ background: '#0d0d14', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['产品', '分类', '价格', '状态', '操作'].map(function(h) {
                      return <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', color: '#3a3a5a', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid #1a1a2e' }}>{h}</th>;
                    })}</tr>
                  </thead>
                  <tbody>
                    {products.filter(function(p) { return !searchQuery || p.name.includes(searchQuery) || p.category.includes(searchQuery); }).map(function(product) {
                      return (
                        <tr key={product.id}>
                          <td style={{ padding: '12px 16px', borderBottom: '1px solid #13131e' }}>
                          ...
                          <td style={{ padding: '12px 16px', borderBottom: '1px solid #13131e' }}>
                            <span style={{ background: product.category === '窗户' ? '#13132a' : product.category === '门' ? '#0a2a1a' : '#2a1a0a', color: product.category === '窗户' ? '#818cf8' : product.category === '门' ? '#34d399' : '#fbbf24', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>{product.category}</span>
                          </td>
          ...
          {activeNav === 'quotes' && (
            <div>
              ...
              <div style={{ background: '#0d0d14', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['客户', '联系方式', '产品', '状态', '时间', '操作'].map(function(h) {
                      return <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', color: '#3a3a5a', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid #1a1a2e' }}>{h}</th>;
                    })}</tr>
                  </thead>
                  <tbody>
                          ...
                          <td style={{ padding: '12px 16px', borderBottom: '1px solid #13131e' }}>
                            <select value={quote.status}
                              onChange={async function(e) {
                                ...
                                await fetch('https://window-server.onrender.com/api/quotes/' + quote.id, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }) });
                                setQuotes(quotes.map(function(q) { return q.id === quote.id ? Object.assign({}, q, { status: newStatus }) : q; }));
                              }}
                              style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '6px', color: quote.status === '待跟进' ? '#fbbf24' : quote.status === '已联系' ? '#34d399' : '#818cf8', padding: '4px 8px', fontSize: '11px', cursor: 'pointer', outline: 'none' }}>
